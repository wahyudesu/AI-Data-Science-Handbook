## Poin yang ada pada tiap-tiap bab
- paragraf: penjelasan, penjabaran teori
- bullet list : resources, checklist 
- table: perbandingan, penjelasan dari jenis jenis
- gambar: intinya gambar punya orang untuk mempermudah memahami sesuatu
- kode: kode contoh penggunaan library/framework, penggunaan 
- quotes: bisa berupa recommendation, question


### Alur isi tiap bab

- pengantar: latar belakang
- bab 1 - data: cara mendapatkan data, checklist dan next step
- bab 2 - data preprocesssing: penjabaran teknik berdasarkan skenario yang sering ditemui di dataset
- bab 2 - EDA: jenis dulu baru penjabarannya, lalu ada gambar plotnya
- bab 2 - statistical analysis: banyak teori nya
- bab 2 - data mining: contoh project kaggle dan 
- bab 3 - ml: penjelasan umum
- bab 3 - hyperparameter: banyak teorinya sama kayak statistical analysis
- bab 3 - ensemble: paling daging, banyak teori sekaligus prakteknya

jenis-jenis lalu penjabarannya
- ensemble: ringkasan 
- penutup: rekomendasi buku yang bisa dibaca